...
async def get_relationship_status(
    current_user_id: int, target_user_id: int, db: AsyncSession
) -> str:
    """
...
async def search_users(
    ...
    """
    Search users by username only with pagination.
    ...
    result = await db.execute(
        ...
        .where(
            ...
            User.username.ilike(f"%{query}%"),  # Search by username only
        )
        .offset(offset)
        .limit(limit + 1)
    )
    ...
    return UserSearchResponse(
        users=search_results, total=len(search_results), has_more=has_more
    )
...
async def follow_user(
    ...
    follow_status = (
        FollowStatus.PENDING.value
        if target_user.is_private
        else FollowStatus.ACTIVE.value
    )

    follow = Follow(
        follower_id=current_user.id, following_id=user_id, status=follow_status
    )
    db.add(follow)

    # Create notification
    notification = Notification(
        user_id=user_id, actor_id=current_user.id, type=NotificationType.FOLLOW.value
    )
    db.add(notification)
    ...
    if follow_status == FollowStatus.ACTIVE.value:
        return FollowActionResponse(
            success=True, relationship_status="following", message="Now following"
        )
    else:
        return FollowActionResponse(
            success=True,
...
async def unfollow_user(
    ...
    return FollowActionResponse(
        success=True, relationship_status="none", message="Unfollowed successfully"
    )
...
async def accept_follow_request(
    ...
    """
    result = await db.execute(select(Follow).where(Follow.id == request_id))
    follow = result.scalar_one_or_none()
    ...
    return FollowActionResponse(
        success=True, relationship_status="following", message="Friend request accepted"
    )
...
async def reject_follow_request(
    ...
    return FollowActionResponse(
        success=True, relationship_status="none", message="Request rejected"
    )
...
async def cancel_sent_request(
    ...
    return FollowActionResponse(
        success=True, relationship_status="none", message="Request cancelled"
    )
...
async def get_followers(
    ...
    result = await db.execute(
        ...
        .where(
            Follow.following_id == user_id, Follow.status == FollowStatus.ACTIVE.value
        )
    )
...
async def get_following(
    ...
    result = await db.execute(
        ...
        .where(
            Follow.follower_id == user_id, Follow.status == FollowStatus.ACTIVE.value
        )
    )
