...
async def create_or_get_conversation(
    ...
    if data.user_id == current_user.id:
        raise HTTPException(
            status_code=400, detail="Cannot start conversation with yourself"
        )
    ...
    result = await db.execute(
        select(Conversation).where(
            Conversation.id.in_(subq1), Conversation.id.in_(subq2)
        )
    )
...
async def get_messages(
    ...
    # Build query - include reactions and reaction users
    query = (
        select(Message)
        .options(
    ...
    for msg in reversed(messages):  # Return in chronological order
        ...
        reactions = [
            MessageReactionResponse(
                ...
                emoji=r.emoji,
                user_id=r.user_id,
                user=UserMinimal.model_validate(r.user) if r.user else None,
...
async def send_message(
    ...
    # Create message
    message = Message(
        conversation_id=conversation_id, sender_id=current_user.id, content=data.content
    )
    db.add(message)
